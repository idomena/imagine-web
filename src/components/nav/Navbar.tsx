...
export function Navbar() {
  ...
  return (
    <>
        ...
        <nav className={cn(
          ...
          <div className="flex shrink-0 items-center justify-center px-5 pointer-events-none">
            <Link
              ...
              className="pointer-events-auto flex h-10 w-10 items-center justify-center rounded-full text-white active:scale-95 transition-all duration-150 shadow-[0_2px_16px_rgba(14,165,233,0.40)]"
          ...
          <div className="relative z-30 flex flex-1 items-center justify-end gap-2 min-w-0">
            {isLoading ? (
              ...
              <>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive('/dashboard') ? 'bg-sky-50 text-sky-700' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900',
                  )}
                >
                  <LayoutDashboard className="h-3.5 w-3.5" aria-hidden />
                  Dashboard
                </Link>
                <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-sky-100 bg-sky-50/60 p-1 pl-1.5 pr-3">
                  {user.avatarUrl ? (
                    ...
                    <div className="flex h-[26px] w-[26px] items-center justify-center rounded-full bg-sky-100 text-[10px] font-bold text-sky-700 uppercase select-none">
                      {displayLabel[0] ?? '?'}
                    </div>
                  )}
                  <span className="max-w-[88px] truncate text-xs font-medium text-slate-700">{displayLabel}</span>
                </div>
                <button
              ...
              <>
                <Link href="/sign-in" className="shrink-0 px-4 py-1.5 text-sm font-medium text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
                  Log In
                </Link>
                <Link
                  href="/sign-up"
      ...
      ════════════════════════════════════════════════════════════════════ */}
      <nav
        className="sm:hidden fixed bottom-0 inset-x-0 z-50 border-t border-sky-100/70 bg-white/90 backdrop-blur-xl"
        ...
        <div className="flex items-center h-16 px-1">
          ...
          <div className="flex flex-1 items-center justify-center">
            <Link
              ...
              aria-label="Submit an app"
              data-tutorial="nav-submit"
              className="flex h-12 w-12 items-center justify-center rounded-full text-white active:scale-95 transition-all duration-150 shadow-[0_4px_20px_rgba(14,165,233,0.40)] -mt-5"
              style={{ background: 'linear-gradient(135deg, #0EA5E9, #14B8A6)' }}
            >
          ...
          {isLoading ? (
            ...
            <Link
              ...
              className={cn(
                ...
                isActive('/dashboard') ? 'text-sky-600' : 'text-slate-400',
              )}
            >
              <span className={cn('flex items-center justify-center rounded-xl p-0.5 transition-colors', isActive('/dashboard') && 'bg-sky-50')}>
                {user.avatarUrl ? (
                  ...
                  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-sky-100 text-[9px] font-bold text-sky-700 uppercase select-none">
                    {displayLabel[0] ?? '?'}
                  </div>
                )}
              </span>
...
function BottomTab({ href, label, active, children, dataTutorial }: {
  ...
  return (
    ...
    >
      <span className={cn('flex items-center justify-center rounded-xl p-1 transition-colors', active && 'bg-sky-50')}>
        {children}
      </span>
